...
class AdidasExtractor(BaseExtractor):
    ...
    def extract_category(self, search_term: str, fixed_config: dict) -> List[BaseShoe]:
        ...
        product_list = []
        page_num = 0
        logger.debug(f"Starting extraction for category '{search_term}' using API...")
        ...
        while True:
            ...
            logger.debug(f"Found {len(products)} products on page {page_num+1} for category '{search_term}'.")

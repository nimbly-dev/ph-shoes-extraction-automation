...
class HokaQuality:
    ...
    def test_set_nulls_to_none(self, df: pd.DataFrame) -> bool:
        df2 = df.copy()
        # pick a column other than 'id'
        cols = [c for c in df2.columns if c != "id"]
        col = cols[0]
        # ensure there is at least one missing value to convert
    ...
    def test_convert_price_columns(self, df: pd.DataFrame) -> bool:
        ...
        return all(pd.api.types.is_numeric_dtype(out[col]) for col in ["price_sale","price_original"] if col in out)
